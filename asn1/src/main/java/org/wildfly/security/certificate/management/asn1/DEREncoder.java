...
import org.wildfly.security.certificate.management.asn1._private.CertMgmtMessages;
...
public class DEREncoder implements ASN1Encoder {
    ...
    public void startSet() {
        startConstructedElement(ASN1.SET_TYPE);
    }
    ...
    public void startExplicit(int number) {
        startExplicit(ASN1.CONTEXT_SPECIFIC_MASK, number);
    }

    @Override
    public void startExplicit(int clazz, int number) {
        int explicitTag = clazz | ASN1.CONSTRUCTED_MASK | number;
    ...
    public void endSequence() throws IllegalStateException {
        EncoderState lastState = states.peekLast();
        if ((lastState == null) || (lastState.getTag() != ASN1.SEQUENCE_TYPE)) {
            throw CertMgmtMessages.log.noSequenceToEnd();
        }
    ...
    public void endExplicit() throws IllegalStateException {
        EncoderState lastState = states.peekLast();
        if ((lastState == null) || (lastState.getTag() == ASN1.SEQUENCE_TYPE)
                || (lastState.getTag() == ASN1.SET_TYPE) || ((lastState.getTag() & ASN1.CONSTRUCTED_MASK) == 0)) {
            throw CertMgmtMessages.log.noExplicitlyTaggedElementToEnd();
        }
        endConstructedElement();
    }
    ...
    private void endSet(Comparator<EncoderState> comparator) {
        EncoderState lastState = states.peekLast();
        if ((lastState == null) || (lastState.getTag() != ASN1.SET_TYPE)) {
            throw CertMgmtMessages.log.noSetToEnd();
        }
    ...
    public void encodeOctetString(byte[] str) {
        writeElement(ASN1.OCTET_STRING_TYPE, str);
    }
    ...
    public void encodeIA5String(byte[] str) {
        writeElement(ASN1.IA5_STRING_TYPE, str);
    }
    ...
    public void encodePrintableString(final byte[] str) {
        ...
        writeElement(ASN1.PRINTABLE_STRING_TYPE, str);
    }
    ...
    public void encodePrintableString(final String str) {
        ...
        writeElement(ASN1.PRINTABLE_STRING_TYPE, str.getBytes(StandardCharsets.US_ASCII));
    }

    @Override
    public void encodeUTF8String(final String str) {
        writeElement(ASN1.UTF8_STRING_TYPE, str.getBytes(StandardCharsets.UTF_8));
    }

    @Override
    public void encodeBMPString(final String str) {
        // technically this may fail if str contains a code point outside of the BMP
        writeElement(ASN1.BMP_STRING_TYPE, str.getBytes(StandardCharsets.UTF_16BE));
    }
    ...
    public void encodeUniversalString(final String str) {
        writeElement(ASN1.UNIVERSAL_STRING_TYPE, str.getBytes(UTF_32BE));
    }
    ...
    public void encodeBitString(byte[] str, int numUnusedBits) {
        ...
        writeElement(ASN1.BIT_STRING_TYPE, contents);
    }
    ...
    public void encodeGeneralizedTime(final ZonedDateTime time) {
        writeElement(ASN1.GENERALIZED_TIME_TYPE, GENERALIZED_TIME_FORMAT.format(time).getBytes(StandardCharsets.UTF_8));
    }
    ...
    public void encodeObjectIdentifier(String objectIdentifier) throws ASN1Exception {
        if (objectIdentifier == null || objectIdentifier.length() == 0) {
            throw CertMgmtMessages.log.asnOidMustHaveAtLeast2Components();
        }
                ...
                if (t > LARGEST_UNSHIFTED_LONG) {
                    ...
                    for (;;) {
                        ...
                        if (Character.isDigit(c)) {
                            ...
                            throw CertMgmtMessages.log.asnInvalidOidCharacter();
                        }
                        if (idx == len) {
                            if (numComponents == 0) {
                                throw CertMgmtMessages.log.asnOidMustHaveAtLeast2Components();
                            ...
                            encodeOIDComponent(bi, contents, numComponents, first);
                            writeElement(ASN1.OBJECT_IDENTIFIER_TYPE, contents);
                            return;
                        }
                    }
                } else {
    ...
    public void encodeNull() {
        writeElement(ASN1.NULL_TYPE, NULL_CONTENTS);
    }

    @Override
    public void encodeImplicit(int number) {
        encodeImplicit(ASN1.CONTEXT_SPECIFIC_MASK, number);
    }
    ...
    public void encodeBoolean(final boolean value) {
        writeElement(ASN1.BOOLEAN_TYPE, value ? BOOLEAN_TRUE_AS_BYTES : BOOLEAN_FALSE_AS_BYTES);
    }
    ...
    private int validateFirstOIDComponent(long value) throws ASN1Exception {
        if (value < 0 || value > 2) {
            throw CertMgmtMessages.log.asnInvalidValueForFirstOidComponent();
        }
        return (int) value;
    }
    ...
    private void validateSecondOIDComponent(long second, int first) throws ASN1Exception {
        if ((first < 2) && (second > 39)) {
            throw CertMgmtMessages.log.asnInvalidValueForSecondOidComponent();
        }
    }
    ...
    private int writeLength(int length, ByteStringBuilder dest) throws ASN1Exception {
        int numLengthOctets;
        if (length < 0) {
            throw CertMgmtMessages.log.asnInvalidLength();
        } else if (length <= 127) {
            // Short form
            numLengthOctets = 1;
        } else {
