...
public class DERDecoder implements ASN1Decoder {
    ...
    public void startSequence() throws ASN1Exception {
        readTag(ASN1.SEQUENCE_TYPE);
        int length = readLength();
        states.add(new DecoderState(ASN1.SEQUENCE_TYPE, bi.getIndex() + length));
    }
    ...
    public void startSet() throws ASN1Exception {
        readTag(ASN1.SET_TYPE);
        int length = readLength();
        states.add(new DecoderState(ASN1.SET_TYPE, bi.getIndex() + length));
    }
    ...
    private void endConstructedElement(long nextElementIndex) throws ASN1Exception {
        ...
        if (pos < nextElementIndex) {
            ...
            if (i != (nextElementIndex - pos)) {
                throw CertMgmtMessages.log.asnUnexpectedEndOfInput();
            }
    ...
    public byte[] decodeOctetString() throws ASN1Exception {
        readTag(ASN1.OCTET_STRING_TYPE);
        int length = readLength();
    ...
    public BigInteger decodeBitStringAsInteger() {
        ...
        if (decoder.peekType() != ASN1.INTEGER_TYPE) {
            throw CertMgmtMessages.log.asnUnexpectedTag();
        }
    ...
    public byte[] decodePrintableStringAsBytes() throws ASN1Exception {
        readTag(ASN1.PRINTABLE_STRING_TYPE);
    ...
    public byte[] decodeUniversalStringAsBytes() throws ASN1Exception {
        readTag(ASN1.UNIVERSAL_STRING_TYPE);
        return decodeUncheckedStringAsBytes();
    }
    ...
    public String decodeObjectIdentifier() throws ASN1Exception {
        readTag(ASN1.OBJECT_IDENTIFIER_TYPE);
        int length = readLength();
        int octet;
        long value = 0;
    ...
    public void decodeNull() throws ASN1Exception {
        readTag(ASN1.NULL_TYPE);
        int length = readLength();
        if (length != 0) {
            throw CertMgmtMessages.log.asnNonZeroLengthForNullTypeTag();
        }
    }

    @Override
    public void decodeImplicit(int number) {
        decodeImplicit(ASN1.CONTEXT_SPECIFIC_MASK, number);
    }
    ...
    public boolean decodeBoolean() throws ASN1Exception {
        readTag(ASN1.BOOLEAN_TYPE);
        int length = readLength();
        if (length != 1) {
            throw CertMgmtMessages.log.asnInvalidLengthForBooleanTypeTag();
        }
        if (! bi.hasNext()) {
            throw CertMgmtMessages.log.asnUnexpectedEndOfInput();
        }
    ...
    public boolean isNextType(int clazz, int number, boolean isConstructed) {
        try {
            return peekType() == (clazz | (isConstructed ? ASN1.CONSTRUCTED_MASK : 0x00) | number);
        ...
        try {
            ...
            if (tagNumber == 0x1f) {
                ...
                if ((octet & 0x7f) == 0) {
                    ...
                    throw CertMgmtMessages.log.asnInvalidHighTagNumberForm();
                }
    ...
    private void readTag(int expectedTag) throws ASN1Exception {
        if (implicitTag != -1) {
            expectedTag = implicitTag | (expectedTag & ASN1.CONSTRUCTED_MASK);
            implicitTag = -1;
        }
        long currOffset = bi.getIndex();
    ...
    private int readLength() throws ASN1Exception {
        try {
            ...
            if (length > 127) {
                ...
                if (numOctets > 4) {
                    throw CertMgmtMessages.log.asnLengthEncodingExceeds4bytes();
                }
                length = 0;
                int nextOctet;
